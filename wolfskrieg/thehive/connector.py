...
class theHiveConnector:
    ...
    def _get(self, type, query=''):
        ...
        """
        if type=='job':
            request = self.url+'connector/cortex/job/'+query

        if type=='task':
            request = self.url+'case/task/'+query

        if type=='case':
            request = self.url+'case'+query+'?range=all'

        if type=='alert':
            request = self.url+'alert'+query+"?range=all"

        if type=='log':
            request = self.url+'case/task/'+query+'/log'
    ...
    def _post(self, type, payload, query=''):
        ...
        """
        if type=='alert':
            request = self.url+'alert'

        if type=='alert_search':
            request = self.url+'alert/_search?range=all'

        if type=='artifact':
            request = self.url+'case/artifact/_search?range=all'

        if type=='case':
            request = self.url+'case'

        if type=='task_search':
            request = self.url+'case/task/_search'

        if type=="job":
            request = self.url+'connector/cortex/job'
    ...
    def _patch(self, type, payload, id):
        ...
        """
        if type=='alert':
            request = self.url+'alert/'+id

        response = requests.patch(request, headers=self.post_header, data=payload, verify=False).text
        return response
    ...
    def getAlert(self, alertId=None, time=None, sourceRef=None):
        ...
        if not alertId and not time and not sourceRef:
            ...
            payload={'query': query.Gte("createdAt", time)}
            return self._post('alert_search', json.dumps(payload))

        elif not alertId and not time:
            payload={'query': query.Eq('sourceRef', sourceRef)}
            return self._post('alert_search', json.dumps(payload))
...
#IDEA: Create models for "Alerts", "Cases", and "Artifacts". #20190425 #Standardize
