...
class SentinelOne:
    ...
    def _get(self, type, query=''):
        ...
        """
        if type == 'agents':
            request = self.url+'/agents?limit=100'+query
        ...
        if type == 'threats':
            request = self.url+'/threats?limit=100'+query
            response = json.loads(self.session.get(request, headers=self.headers).text)
        ...
        if type == 'groupedThreats':
            request = self.url+'/private/threat-groups?limit=100'+query
            response = json.loads(self.session.get(request, headers=self.headers).text)
...
# FIXME: getBlackItems. #20190425 #GET
    ...
    def getReputation(self, hash):
        ...
        return int(self._get('reputation', query=hash))
